...
public sealed class GetTransactionsDatesUseCase
    : IUseCase<GetTransactionsByDatesRequest, IEnumerable<TransactionFullResponse>>
{
    private readonly ITransactionRepository _transactionRepository;
    private readonly ICurrentUserService _currentUser;

    public GetTransactionsDatesUseCase(
    ...
    public async Task<ResultEntity<IEnumerable<TransactionFullResponse>>> ExecuteAsync(GetTransactionsByDatesRequest request)
    {
        var userId = _currentUser.UserId;

        var transactions = await _transactionRepository
            .GetByDatesAsync(userId, request.StartDate, request.EndDate);
